...
def classify_images(images_dir, results_dic, model):
     ...
     if pet_label in formatted_classifier_label:
            results_dic[filename].extend([formatted_classifier_label, 1])  # Match
        else:
            results_dic[filename].extend([formatted_classifier_label, 0]) 
...
def classify_images(images_dir, results_dic, model):
    for filename in results_dic:
        # Get the pet image label from results_dic
        pet_label = results_dic[filename][0]
        
        # Call the classifier function and format the result
        classifier_label = classifier(images_dir + filename, model)
        classifier_label = classifier_label.lower().strip()
        
        # Check if pet_label is in formatted_classifier_label
        if pet_label in classifier_label:
            results_dic[filename].extend([classifier_label, 1])  # Match
        else:
    ...
    # Key already exists, extend values to end of list for idx 3 & 4
                results_dic[filename[idx]].extend([pet_label_is_dog[idx],classifier_label_is_dog[idx]])'''
